...
def train_and_fit(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        net.train(); total_loss = 0.0;resutf=0.0;textresult=''; losses_per_batch = []; total_acc = 0.0; lm_accuracy_per_batch = []
        for i, data in enumerate(train_loader, 0):
            ...
            loss = criterion(lm_logits, blanks_logits, masked_for_pred, blank_labels,args, verbose=verbose)
            ...
            resultall= evaluate_(lm_logits, blanks_logits, masked_for_pred, blank_labels, \
                                   tokenizer, print_=False)
            total_acc +=resultall[0] #evaluate_(lm_logits, blanks_logits, masked_for_pred, blank_labels, \
            #                       tokenizer, print_=False)[0]
            resutf+=resultall[2].min().cpu().detach().numpy().tolist()
            
            if (i % update_size) == (update_size - 1):
                losses_per_batch.append(args.gradient_acc_steps*total_loss/update_size)
                lm_accuracy_per_batch.append(total_acc/update_size)
                print('[Epoch: %d, %5d/ %d points] total loss, lm accuracy per batch: %.3f, %.3f,%.3f' %
                      (epoch + 1, (i + 1), train_len, losses_per_batch[-1], lm_accuracy_per_batch[-1],resutf/update_size))
                ...
                total_loss = 0.0; total_acc = 0.0;resutf=0.0
        ...
        if(len(lm_accuracy_per_batch)!=0):
        ...
        if (epoch % 1) == 0:
            ...
            save_as_pickleResult("test_accuracy_per_epoch_%d.pkl" % args.model_no, accuracy_per_epoch,args)
            torch.save({
                    'epoch': epoch + 1,\
                    'state_dict': net.state_dict(),\
                    'best_acc': accuracy_per_epoch[-1],\
                    ...
                    'scheduler' : scheduler.state_dict(),\
                    'amp': amp.state_dict() if amp is not None else amp
                }, os.path.join(args.ResultPathDataset , "test_checkpoint_%d.pth.tar" % args.model_no))
    ...
    plt.savefig(os.path.join(args.ResultPathDataset ,"loss_vs_epoch_%d.png" % args.model_no))
    ...
    plt.savefig(os.path.join(args.ResultPathDataset ,"accuracy_vs_epoch_%d.png" % args.model_no))
