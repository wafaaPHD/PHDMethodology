...
class Two_Headed_Loss(nn.Module):
    ...
    def forward(self, lm_logits, blank_logits, lm_labels, blank_labels,args, verbose=False):
        '''
        ...
        if(len(lm_logits)==len(lm_labels)):
        ...
        if verbose:
            ...
            with open(args.ResultPathDataset+'lossLm.txt', 'a') as f:
                if(lm_loss==0):
                    f.write(str(lm_loss))
                    f.write("\n")
                else:                      
                      f.write(str(lm_loss.item()))
                      f.write("\n")
            with open(args.ResultPathDataset+'blank_loss.txt', 'a') as f:
                      f.write(str(blank_loss.item()))
                      f.write("\n")
            with open(args.ResultPathDataset+'total_loss.txt', 'a') as f:
                      f.write(str((lm_loss + blank_loss).item()))
                      f.write("\n")
...
def load_results(args):
    ...
    accuracy_path =args.ResultPathDataset+ "test_accuracy_per_epoch_%d.pkl" % args.model_no
    if os.path.isfile(losses_path) and os.path.isfile(accuracy_path):
        ...
        accuracy_per_epoch = load_pickleResult("test_accuracy_per_epoch_%d.pkl" % args.model_no,args)
...
def evaluate_(lm_logits, blanks_logits, masked_for_pred, blank_labels, tokenizer, print_=True):
    ...
    lm_logits_pred_ids1=lm_logits_pred_ids
    masked_for_pred1=masked_for_pred
    padded = pad_sequence([lm_logits_pred_ids,masked_for_pred],batch_first=True)
    lm_logits_pred_ids=padded[0]
    masked_for_pred=padded[1]
    ...
    Accuracy = accuracy_score(masked_for_pred.cpu(), lm_logits_pred_ids.cpu())
     
    return lm_accuracy, blanks_mse,multiclass_f1_score(masked_for_pred, lm_logits_pred_ids, num_classes=len(lm_logits_pred_ids))
